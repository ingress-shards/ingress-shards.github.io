...
def generate_flag_assets(series_geocode):
    country_codes = set()
    for series in series_geocode.values():
        for site in series.get('sites', []):
            cc = site.get('country_code')
            if cc:
                country_codes.add(str(cc).lower())

    gen_dir = os.path.join(SCRIPT_DIR, '..', 'gen')
    flags_dest_dir = os.path.join(gen_dir, 'flags')
    os.makedirs(flags_dest_dir, exist_ok=True)

    flags_source_dir = os.path.join(SCRIPT_DIR, '..', 'node_modules', 'flag-icons', 'flags', '4x3')

    copied_count = 0
    css_rules = [
        "/* Generated by geocode.py - do not edit manually */",
        ".fi {",
    ...
    for cc in sorted(country_codes):
        src_svg = os.path.join(flags_source_dir, f"{cc}.svg")
        dest_svg = os.path.join(flags_dest_dir, f"{cc}.svg")
        if os.path.exists(src_svg):
            shutil.copyfile(src_svg, dest_svg)
            copied_count += 1
            css_rules.append(f'.fi.fi-{cc} {{ background-image: url("./flags/{cc}.svg"); }}')
        else:
            print(f"Warning: Flag SVG not found for country code '{cc}' at {src_svg}")

    css_path = os.path.join(gen_dir, 'flag-icons.css')
    with open(css_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(css_rules) + '\n')

    print(f"Generated {css_path} and copied {copied_count} flag SVGs to {flags_dest_dir}.")
...
def main():
    ...
    generate_flag_assets(series_geocode)
